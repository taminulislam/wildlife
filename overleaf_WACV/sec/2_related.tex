...
\paragraph{Object detection for small targets.}
The one-stage/two-stage families are both represented in our benchmark: Faster
...
\cref{sec:counting}. Critically, MOT benchmarks score association quality (MOTA, IDF1,
...
\paragraph{Animal detection and counting.}
Camera-trap classification at scale~\cite{norouzzadeh2018automatically} and the
domain-shift benchmarks that followed~\cite{beery2018recognition} established that animal
imagery is its own distribution problem. Aerial and UAV surveys have received the most
counting-specific attention~\cite{kellenberger2018detecting,corcoran2019automated}, usually
with per-image counts on sparse still imagery rather than per-individual identity across
continuous video. Thermal sensing is attractive for nocturnal surveys because it is
illumination-independent, but thermal wildlife datasets with \emph{per-individual track}
annotation over long sequences remain scarce; that scarcity is what our corpus addresses.

\paragraph{Counting as its own task.}
...
\paragraph{Animal re-identification.}
Individual identification from appearance is established for species with high-contrast,
high-resolution natural markings. A co-author asked whether the same could be done here, and
\cref{sec:analysis} answers it quantitatively rather than by assertion: at a median 27-pixel
animal, appearance carries less identity signal than a two-number size descriptor, and an
ImageNet backbone scores \emph{below} plain geometry. We report this as a measured
resolution limit.
