...
rather than in place of them. The two disagree sharply, and the disagreement is the paper's
starting point: \textbf{every one of the twelve architectures we train finds between 82\% and
99.6\% of the 235 annotated individuals in at least one frame, while none exceeds
AP$_\mathbf{50}$ 0.510 on the same images.} Per-box mAP is not wrong here. It is answering a
question the survey does not ask, and a reader who judges these detectors by it will conclude
that detection is the problem when the measurements say it is very nearly solved.
