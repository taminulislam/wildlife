...
The split is a threshold choice rather than a capability limit. Scored at confidence 0.10
instead of 0.25, the same YOLO12m weights give 0.711 precision and 0.653 recall for an F1
of 0.681 --- twenty points of recall bought for twenty-two of precision, one knob on one
curve. This also means the recall figure in \cref{tab:detectors} is not the operating point
of the system this paper describes: \textbf{the counting pipeline runs the detector at
confidence 0.10}, and 0.25 is reported only because it is the conventional threshold for a
detection table. The direction of the trade matters more than its size, because it is not
symmetric. A quiet detector can be made louder, but a loud one has nowhere left to go: at
0.10 RTMDet-m emits 155,491 false boxes for 2611 true ones, a precision of 0.017, and no
confirmation stage can recover from a pool of that composition --- which is the argument of
\cref{sec:inversion} appearing one stage earlier. Recall is understated by the matching
rule as well: relaxing IoU$\geq$0.50 to presence cuts YOLO12m's false positives from 307 to
96 while its misses fall only from 1741 to 1530, because a looser criterion can credit a box
placed a few pixels off an animal but cannot recover an animal the detector never fired on.
