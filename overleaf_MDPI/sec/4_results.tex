...
conventional $0.5$ rather than tuned. The final column is different in kind from the rest:
\textbf{Animals Found} is the fraction of the 235 annotated individuals the detector fires on
in at least one frame, so it counts \emph{animals} where every other column counts boxes. Twelve architectures were trained; Deformable DETR R50
...
The last column of \cref{tab:detectors} makes that gap explicit for every model. Measured per
...
\end{adjustwidth}
  \caption{\textbf{Validation curves for the six strongest detectors.} One panel per
  detector, drawn from the training log each run wrote; no curve is smoothed or re-fitted.
  Panels are ordered by test AP$_{50}$, strongest first, and truncated at epoch 70 so the
  curves are read against a common range; YOLO11m alone ran longer, to 110. The $y$-axes are
  not shared. Two patterns matter for what follows. Precision settles near 0.65--0.70 while
  recall settles near 0.40--0.45 in every panel, which is the precision-heavy operating point
  \cref{sec:choosing} argues the counting stage actually needs; and several models
  (\textbf{a}, \textbf{c}, \textbf{f}) peak early and then decline, so the figures we report
  come from the best checkpoint rather than the last. The five mmdetection models of
  \cref{tab:detectors} are absent for a reason worth stating: their validation loop logs only
  the COCO evaluator's output, which contains no scalar precision or recall, so they could
  contribute only a different set of series. A uniform figure of one framework carries the
  operating-point argument better than a ragged one of two, and the benchmark table reports
  all eleven models regardless.\label{fig:training}}
\end{figure}
