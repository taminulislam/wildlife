...
limit.} The same three sets of weights scored on the same test split under the presence
criterion, at the conventional reporting threshold of 0.25 and at the 0.10 the counting
pipeline actually runs the detector at. Lowering the threshold buys YOLO12m 20 points of
recall for 22 of precision, which is one knob on one curve. The RTMDet-m rows show why the
direction matters: a detector that is quiet can be made louder, but a detector that is
already loud has nowhere to go --- at 0.10 it emits 155,491 false boxes for 2611 true ones,
and no confirmation stage can recover from that.\label{tab:threshold}}
\centering\small
