...
actually runs the detector at. \textbf{Models are ordered by the best F1 either threshold
achieves, strongest first}; the same ordering follows from precision. The three are the two
strongest detectors of \cref{tab:detectors} together with the most recall-oriented one, so
the rows span the operating range rather than sampling it arbitrarily. Lowering the
