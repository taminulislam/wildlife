...
\end{adjustwidth}
  ...
  and each panel carries its own legend: the Ultralytics runs (\textbf{a}--\textbf{d},
  \textbf{f}, \textbf{g}) log precision, recall and mAP@50 per epoch, from which F1 is
  computed, while the mmdetection ones (\textbf{e}, \textbf{h}) log the COCO mAP family.
  Panels are ordered by test AP$_{50}$, strongest first, which interleaves the two
  frameworks. All are truncated at epoch 70, the length of the mmdetection schedules, so the
  ...
  recall settles near 0.40--0.45 in every Ultralytics panel, which is the operating point
  \cref{sec:choosing} argues the counting stage actually needs; and several models
  (\textbf{c}, \textbf{e}, \textbf{g}) peak early and then decline, so the reported figures
  come from the best checkpoint rather than the last. These are the eight strongest detectors
  of the eleven in \cref{tab:detectors}; ATSS R50, TOOD R50, YOLOv8m and DINO R50 are omitted
  for legibility, and Deformable DETR R50 produced no curve because its training ran out of
  GPU memory at epoch 5.\label{fig:training}}
\end{figure}
