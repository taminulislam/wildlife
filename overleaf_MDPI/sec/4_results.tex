...
\end{adjustwidth}
  ...
  and each panel carries its own legend: the Ultralytics models (\textbf{a}--\textbf{f})
  log precision, recall and mAP@50 per epoch, from which F1 is computed, while the
  mmdetection models (\textbf{g}--\textbf{j}) log the COCO mAP family. All panels are
  truncated at epoch 70, the length of the mmdetection schedules, so that the curves are read
  against a common range; YOLO11m alone ran longer, to 110. The $y$-axes are not shared.
  ...
  (\textbf{b}, \textbf{e}, \textbf{h}) peak early and then decline, so the reported figures
  come from the best checkpoint rather than the last. Three of the benchmarked models are
  omitted for space and legibility --- YOLOv8m and DINO R50, whose curves add nothing the
  others do not show, and Deformable DETR R50, whose training ran out of GPU memory at epoch
  5 and has no curve to draw (\cref{tab:detectors}).\label{fig:training}}
\end{figure}
