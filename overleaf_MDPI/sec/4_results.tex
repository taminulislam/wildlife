...
is excluded because it did not converge within our compute budget.\label{tab:detectors}}
...
limit.} Three sets of weights scored on the same test split under the presence criterion,
at the conventional reporting threshold of 0.25 and at the 0.10 the counting pipeline
actually runs the detector at. All six detectors we trained under one framework, at
two thresholds each, ordered by the best F1 either threshold achieves. That ordering is for
reading, not a verdict: F1 weights precision and recall equally, which \cref{sec:choosing}
argues is the wrong balance for a counting pipeline, and under the precision-weighted
$F_{0.5}$ we actually select on, YOLO11m leads at 0.775 against YOLO12m's 0.767. Lowering the
threshold buys YOLO12m 20 points of recall for 22 of precision, which is one knob on one
curve. The direction of the trade matters more than its size, because it is not
symmetric, and the lower rows show it. A detector already operating at high recall has
nowhere to go: dropping RT-DETR-L to 0.10 buys 231 additional true boxes and costs 7433
false ones, collapsing precision to 0.187, and RTMDet-m --- omitted here as the extreme case
--- reaches 155,491 false boxes at the same threshold. Neither is a candidate pool any
confirmation stage can work with, which is the argument of \cref{sec:inversion} appearing
one stage earlier.\label{tab:threshold}}
\centering\small
...
\multirow{2}{*}{YOLO12m} & 0.25 & \textbf{0.929} & 0.452 & 0.608 & 1262 & \textbf{96} & 1530 \\
 & 0.10 & 0.711 & 0.653 & \textbf{0.681} & 1824 & 741 & 968 \\
\midrule
\multirow{2}{*}{YOLO11m} & 0.25 & 0.897 & 0.503 & 0.644 & 1404 & 162 & 1388 \\
 & 0.10 & 0.777 & 0.576 & 0.661 & 1608 & 462 & 1184 \\
\midrule
\multirow{2}{*}{YOLOv8m} & 0.25 & 0.780 & 0.572 & 0.660 & 1598 & 450 & 1194 \\
 & 0.10 & 0.361 & \textbf{0.760} & 0.490 & 2121 & 3749 & \textbf{671} \\
\midrule
\multirow{2}{*}{YOLOv9m} & 0.25 & 0.917 & 0.450 & 0.604 & 1256 & 114 & 1536 \\
 & 0.10 & 0.769 & 0.572 & 0.656 & 1596 & 478 & 1196 \\
\midrule
\multirow{2}{*}{YOLOv10m} & 0.25 & 0.874 & 0.453 & 0.597 & 1266 & 182 & 1526 \\
 & 0.10 & 0.661 & 0.615 & 0.637 & 1717 & 882 & 1075 \\
\midrule
\multirow{2}{*}{RT-DETR-L} & 0.25 & 0.643 & 0.605 & 0.623 & 1689 & 939 & 1103 \\
 & 0.10 & 0.187 & 0.688 & 0.293 & 1920 & 8372 & 872 \\
\bottomrule
\end{tabularx}
\end{table}
...
\end{adjustwidth}
  ...
  mmdetection models (\textbf{h}--\textbf{l}) log the COCO mAP family. Epoch counts differ
  because runs early-stopped at different points under a fixed compute budget; the
  $x$-axes are therefore not comparable across panels and the $y$-axes are not shared.
  ...
  come from the best checkpoint rather than the last. Deformable DETR R50 is absent because
  it reached only two validation epochs within the same budget, which is the exclusion
  noted in \cref{tab:detectors}.\label{fig:training}}
\end{figure}
...
The remedy is less straightforward than it appears, though not for the reason we first
believed. The obvious fix is to widen scale augmentation until the training distribution
reaches 100--200\,px. Two attempts to do so collapsed to near-zero mAP within a few epochs,
and we initially attributed that to the symmetry of the standard implementation: sampling
from $[1-m,\,1+m]$ means that reaching $2.2\times$ also means sampling $0\times$, which
drives a 27\,px animal to sub-pixel size on a corpus that is 71\% small objects. On
re-examination that attribution is not supported. The second run moved three
hyperparameters at once --- scale $0.5\!\to\!0.6$, mosaic $1.0\!\to\!0.4$ and batch
$32\!\to\!48$ --- and its mAP@50 fell $0.406\to0.138\to0.001$ over epochs one to three, which
is where the three-epoch learning-rate warm-up completes. That trajectory is characteristic
of an optimizer divergence rather than of a gradual augmentation penalty, and a scale of
0.6 is in any case barely wider than the 0.5 under which the same model trains stably for
110 epochs. We therefore report the collapse as an observation and not as evidence about
augmentation.
...
mosaic augmentation does not shrink objects as we previously stated --- the implementation
composes a $2\times$ canvas and crops it back to the input size, so object pixel dimensions
are preserved. The interaction we described between mosaic and scale therefore does not
exist. A controlled experiment isolating the scale parameter, with every other setting
matched to the baseline run, is the correct test and is the natural next step; we report
this failure honestly rather than presenting an untested mechanism for it.
