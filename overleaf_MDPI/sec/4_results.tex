...
precision and recall equally --- the balance \cref{sec:choosing} argues is wrong for a counting
pipeline. Under the precision-weighted $F_{0.5}$ we actually select on, YOLO11m leads at 0.775
...
RT-DETR-L to 0.10 buys 231 additional true boxes and costs 7433 false ones, collapsing
precision to 0.187 --- not a candidate pool any confirmation stage can work with, which is the
...
Deformable DETR's training exhausted GPU memory at epoch 5 and was not rerun at a smaller batch. RT-DETR-L, which targets that weakness,
...
missing all three is a complete failure, and the mean absolute error of 2.38 animals per video
in \cref{tab:counting} conceals that a small-count transect can be wrong by 100\% while
...
and the substitution is a poor one: at $c=0$ the held-out MAE is 3.69 against 2.38, and
...
copies, so the detector is not strictly naive to large objects, but under the symmetric
