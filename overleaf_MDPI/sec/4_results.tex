...
actually runs the detector at. The two rows per model are the two strongest
detectors of \cref{tab:detectors}, ordered by the best F1 either threshold achieves. That
ordering is for reading, not a verdict: F1 weights precision and recall equally, which
\cref{sec:choosing} argues is the wrong balance for a counting pipeline, and under the
precision-weighted $F_{0.5}$ we actually select on, YOLO11m leads YOLO12m at 0.775 to
0.767. Lowering the
...
symmetric. A quiet detector can be made louder; a loud one has nowhere to go. Scored at
0.10, the most recall-oriented model in \cref{tab:detectors}, RTMDet-m, emits 155,491 false
boxes for 2611 true ones --- a precision of 0.017, and not a candidate pool any confirmation
stage can work with.\label{tab:threshold}}
\centering\small
...
\multirow{2}{*}{YOLO12m} & 0.25 & \textbf{0.929} & 0.452 & 0.608 & 1262 & 96 & 1530 \\
 & 0.10 & 0.711 & \textbf{0.653} & \textbf{0.681} & 1824 & 741 & 968 \\
\midrule
