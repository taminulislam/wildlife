...
actually runs the detector at. Models are ordered by the best F1 either threshold
achieves, highest first. That is an ordering for reading the table, not a verdict: F1
weights precision and recall equally, which \cref{sec:choosing} argues is the wrong balance
for a counting pipeline, and under the precision-weighted $F_{0.5}$ we actually select on,
YOLO11m leads YOLO12m at 0.775 to 0.767. The three rows are the two strongest detectors of
\cref{tab:detectors} together with the most recall-oriented one, so they span the operating
range rather than sampling it arbitrarily. Lowering the
threshold buys YOLO12m 20 points of recall for 22 of precision, which is one knob on one
curve. The RTMDet-m rows show why the direction matters: a detector that is quiet can be
made louder, but a detector that is already loud has nowhere to go --- at 0.10 it emits
155,491 false boxes for 2611 true ones, and no confirmation stage can recover from
that.\label{tab:threshold}}
\centering\small
...
\midrule
\multirow{2}{*}{YOLO12m} & 0.25 & \textbf{0.929} & 0.452 & 0.608 & 1262 & 96 & 1530 \\
...
\midrule
\multirow{2}{*}{RTMDet-m} & 0.25 & 0.325 & 0.779 & 0.459 & 2176 & 4513 & 616 \\
 & 0.10 & 0.017 & \textbf{0.935} & 0.033 & 2611 & 155,491 & 181 \\
\bottomrule
\end{tabularx}
\end{table}
