...
predicted where 83 exist across the 13 held-out transects, a shortfall of 25. For an abundance estimate that is not the quantity to optimise. Setting $c=0.45$ instead ---
...
(\cref{fig:scale}), a failure mode that a small-object-focused evaluation would never surface. And do not carry
an absolute confidence threshold to a new site: \cref{sec:loso} shows a detector trained
without a site scores that site systematically lower, so a threshold fitted elsewhere can
reject every track. Budget for a small labelled sample from each new site to re-fit it.
