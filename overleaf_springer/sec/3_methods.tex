...
measure it. The pipeline is deliberately conventional; the measurement is not.
...
parameters are fitted by grid search rather than by training. The pipeline also has no
stochastic component at inference. Run three times on the same GPU over four held-out videos
holding 55 of the 83 held-out animals, it produced byte-identical detections, tracks and
counts, so a count is a fixed function of the video rather than a draw from a distribution,
and repeating a run cannot reduce its error. Across platforms, one of the benchmarked detectors
evaluated on an NVIDIA GH200, with a different processor architecture and different PyTorch
and CUDA builds, reproduced its A100 keyframe AP$_{50}$ of 0.670; counts themselves were
compared only on identical hardware. The detector is also not the one
...
detector confidences is at least $c$. The values are not set by hand. They are chosen by a
grid search that minimizes counting error on the 19 detector-training videos, over
$m\in\{1,2,3,5,8,12,20\}$ frames, $s\in\{0,0.1,0.25,0.5,1,2\}$\,s and $c$ from 0 to 0.85,
and are frozen before any held-out video is scored (\cref{sec:heldout}). The search selects
$m\ge20$, $s\ge0$, $c\ge0.65$. At that operating point the span condition imposes nothing,
since $s\ge0$ accepts a track of any duration, so the rule is in effect two parameters; we keep
$s$ because it is the parameter the search uses to compensate when $c$ is lowered
(\cref{sec:sensitivity}), and \cref{fig:rulegrid} shows the whole search. Note that $c$ is a property of the
...
793 low-confidence tracks are flagged for human review rather than auto-counted. We also propagate the per-track probabilities into a Poisson-binomial standard deviation on
each video's count. That quantity is narrower than it looks. It expresses uncertainty about
whether the tracks already accepted are real animals and says nothing about animals that
were never detected or never accepted, so on held-out video its nominal 95\% band contains the
true count in only one of thirteen videos. It should not be read as an interval on abundance,
...
\label{sec:decomposition}
 ...
 Let $G$ be the ground-truth animals in a video and $C$
...
Counting runs process all 32 videos, but the detector trained on 19 of them, and the confirmation rule is conventionally swept on the same videos it is reported on.
...
videos is 81.8\%, and on the 13 held-out videos it is 66.3\%. Four of the 13 held-out videos are the detector's \emph{validation} split. No gradient was computed on them, but
